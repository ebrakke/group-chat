...
export class ChatPage {
  ...
  async openCreateChannelModal() {
    await this.page.click('button:has-text("New Channel"), button:has-text("+")');
    await expect(this.page.locator('div[role="dialog"]').first()).toBeVisible();
  }
  ...
  /**
   * Drag and drop a file into the message input
   */
  async dragAndDropFile(filePath: string) {
    ...
    await this.messageInput.dispatchEvent('drop', { dataTransfer });
  }

  /**
   * Check if an image preview is visible
   */
  async hasImagePreview(filename: string): Promise<boolean> {
    const preview = this.page.locator(`img[alt*="${filename}"], img[src*="${filename}"]`);
    return await preview.count() > 0;
  }

  /**
   * Check if a file download link is visible
   */
  async hasDownloadLink(filename: string): Promise<boolean> {
    const link = this.page.locator(`a[download*="${filename}"], a:has-text("${filename}")`);
